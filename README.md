...
面板 → **设置 → 模型与 API** → 选服务商、填 API Key → 点「测试连接」（结果显示在按钮左侧，成功绿 / 失败红）。修改会自动保存；可用「恢复默认」重置参数（API Key 保留）。
...
```
MayaAgent/
├── config/default_config.yaml   # 默认配置（人类可读）
├── config/default_config.json   # 运行时加载（无第三方依赖）
├── maya_agent/
